...
import { ShadowMapper } from '../../finalproject.shadowmapper.js'
...
class WebGlApp 
{
    ...
    constructor( gl, shaders, app_state )
    {
        // Set GL flags
        this.setGlFlags( gl )
        this.shadowMapper = new ShadowMapper(gl);
    ...
    prepareShadowMap() {
        // You'll need to implement logic to select a light source
        // For example, find the first directional light in the scene
        const light = this.findMainDirectionalLight();
        
        if (!light) return null;

        return this.shadowMapper.prepareShadowMap(
            [1, 1, 1],  // Light position
            [0, 0, 0],       // Scene center (you might want to calculate this)
            1               // Scene extent (you might want to calculate this)
        );
    }
    ...
    renderShadowMap(shadowParams) {
        ...
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.shadowMapper.framebuffer);
        gl.viewport(0, 0, shadowParams.shadowMapSize, shadowParams.shadowMapSize);
        gl.clear(gl.DEPTH_BUFFER_BIT);
    
        // Use the depth shader for shadow map rendering
        depthShader.use();
        depthShader.setUniformMatrix4fv('uLightSpaceMatrix', shadowParams.light_space_matrix);
    
        if (this.scene) {
            const modelNodes = this.scene.getNodes().filter(node => node.type === 'model');
            
            for (let node of modelNodes) {
                node.obj3d.renderShadow(
                    gl, 
                    depthShader, 
                    shadowParams.light_space_matrix
                );
            }
        }
    ...
    renderSceneWithShadows(shadowParams) {
        ...
        gl.bindTexture(gl.TEXTURE_2D, this.shadowMapper.depthTexture);
        mainShader.setUniform1i('uShadowMap', 0); // Texture unit 0
        mainShader.setUniformMatrix4fv('uLightSpaceMatrix', shadowParams.light_space_matrix);
